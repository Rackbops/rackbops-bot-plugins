...
describe("command shape", () => {
  test("link/unlink build to the same JSON as the bot's baked-in commands", async () => {
    ...
    const plugin = createPlugin(makeFakeHost({ name: "warbandeer" }));
  ...
  test("createPlugin throws on an invalid WARBANDEER_INGEST_PORT (host then skips the plugin)", () => {
    expect(() => createPlugin(makeFakeHost({ name: "warbandeer", env: { WARBANDEER_INGEST_PORT: "0" } }))).toThrow(
      /WARBANDEER_INGEST_PORT must be a valid port number, got "0"/,
    );
    expect(() => createPlugin(makeFakeHost({ name: "warbandeer", env: { WARBANDEER_INGEST_PORT: "notaport" } }))).toThrow(
      /must be a valid port number/,
    );
    expect(() => createPlugin(makeFakeHost({ name: "warbandeer", env: { WARBANDEER_INGEST_PORT: "70000" } }))).toThrow(
      /must be a valid port number/,
    );
  });

  test("createPlugin accepts a valid port and an unset port", () => {
    expect(() => createPlugin(makeFakeHost({ name: "warbandeer", env: { WARBANDEER_INGEST_PORT: "8787" } }))).not.toThrow();
    expect(() => createPlugin(makeFakeHost({ name: "warbandeer", env: {} }))).not.toThrow();
  });
});
